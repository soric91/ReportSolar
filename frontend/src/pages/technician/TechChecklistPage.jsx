  ...
  const renderFotoSection = (secId, campo) => {
    ...
    return (
      <div className="mt-4 pt-4 border-t border-gray-200">
        <h5 className="text-xs font-bold text-gray-700 mb-3 flex items-center gap-1.5"><CameraIcon className="w-3.5 h-3.5" /> {modoTriple ? 'Fotos (Antes/Durante/Después)' : esMultiple ? 'Fotos' : 'Foto'}</h5>

        {modoTriple ? (
          <div className="grid grid-cols-3 gap-2">
            {[
              { tipo: 'antes', label: 'Antes', lista: fotosAntes, activo: 'bg-primary-100 text-primary-700 border-primary-300', borde: 'border-primary-300' },
              { tipo: 'durante', label: 'Durante', lista: fotosDurante, activo: 'bg-purple-100 text-purple-700 border-purple-300', borde: 'border-purple-300' },
              { tipo: 'despues', label: 'Después', lista: fotosDespues, activo: 'bg-green-100 text-green-700 border-green-300', borde: 'border-green-300' },
            ].map(({ tipo, label, lista, activo, borde }) => (
              <div key={tipo} className="flex flex-col gap-1.5">
                ...
                {lista.length > 0 && (
                  <div className="flex gap-1 flex-wrap">
                    {lista.map((foto) => (
                      <div key={foto.id} className="relative w-full">
                        <img src={foto.url} alt={label} className={`w-full h-16 object-cover rounded border-2 ${borde}`} />
